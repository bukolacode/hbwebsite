<?php
 ...
 if(isset($_POST['add_room']))
 {
    ...
    $facilities = filteration(json_decode($_POST['facilitites']));
...
if(isset($_POST['edit_room']))
{
  ...
  $del_features = delete("DELETE FROM room_features WHERE room_id=?", [$frm_data['room_data']],'i');
  $del_facilities = delete("DELETE FROM room_facilities WHERE room_id=?", [$frm_data['room_data']],'i');
...
if (isset($_POST['get_room_images'])) 
{
  ...
  {
    if($row['thumb']==1){
      $thumb_btn = "<i class='bi bi-check-lg text-light bg-success px-2 py-1 rouunded fs-5'></i>";
    }
    else {
      $thumb_btn = "<button onclick='thumb_image($row[id_no],$row[room_id])' class='btn btn-secondary  shadow-none'>
    ...
    echo <<<data
      ...
      <td>$thumb_btn</td>
      <td>
        <button onclick='rem_image($row[id_no],$row[room_id])' class='btn btn-danger  shadow-none'>
        <i class='bi bi-trash'></i>
        </button>
      </td>
      </tr>
...
if (isset($_POST['rem_image']))
{
  ...
  $pre_q = "SELECT * FROM room_image WHERE id_no=? AND room_id=?";
  $values = [$frm_data['image_id'],$frm_data['room_id']];
  $res = select($pre_q,$values,'ii');
  $img = mysqli_fetch_assoc($res);

  if(deleteImage($img['image'],ROOMS_FOLDER)){
      $sql = "DELETE FROM room_image WHERE id_no=? AND room_id=?";
      return delete($sql,$values,'ii');
      echo $res;
  }else {
      echo 0;
  }
...
if (isset($_POST['thumb_image']))
{
  ...
  $q = "UPDATE room_image SET `thumb`=? WHERE id_no=? AND room_id=?";
