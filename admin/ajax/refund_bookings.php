...
if(isset($_POST['get_bookings']))
{
  $frm_data = filteration($_POST);
   ...
   $query = "SELECT bo.*, bd.* FROM booking_order bo INNER JOIN booking_details bd ON bo.booking_id = bd.booking_id 
    WHERE (bo.order_id LIKE ? OR bd.phonenum LIKE ? OR bd.user_name LIKE ?) AND bo.booking_status = ? AND bo.refund = ? ORDER BY bo.booking_id ASC";
    ...
    while($data = mysqli_fetch_assoc($res))
    {
      ...
      $table_data .= "
        <tr>
         ...
         <td>
         <button type='button' onclick='refund_booking($data[booking_id])' class='btn btn-success btn-sm text-white fw-bold shadow-none'>
