...
if(isset($_POST['get_bookings']))
{
  $frm_data = filteration($_POST);
   ...
   $query = "SELECT bo.*, bd.* FROM booking_order bo INNER JOIN booking_details bd ON bo.booking_id = bd.booking_id 
    WHERE (bo.order_id LIKE ? OR bd.phonenum LIKE ? OR bd.user_name LIKE ?) AND bo.booking_status =? AND bo.arrival =? ORDER BY bo.booking_id ASC";
      ...
      $table_data .= "
        <tr>
         ...
         <button type='button' onclick='assign_room($data[booking_id])' class='btn btn-sm text-white fw-bold custom-bg shadow-none' data-bs-toggle='modal' data-bs-target='#assign-room'>
          ...
          <br>
         <button type='button' onclick='cancel_booking($data[booking_id])' class='mt-2 btn btn-outline-danger btn-sm text-danger fw-bold shadow-none'>
...
if(isset($_POST['assign_room']))
{
  ...
  $query = "UPDATE booking_order bo INNER JOIN booking_details bd ON bo.booking_id = bd.booking_id
   SET bo.arrival = ?, bo.rate_rewiew = ?, bd.room_no = ? WHERE bo.booking_id = ?";
