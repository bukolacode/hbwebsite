...
if(isset($_GET['fetch_rooms']))
{
   ...
   if($chk_avail['checkin']!='' && $chk_avail['checkout']!='')
   {
    ...
    // count no. of room and output variables to store room card
    $count_rooms = 0;

    $output = "";
    
    // fetching setting table to check website is shutdown or not
    $settings_query = "SELECT * FROM settings WHERE id_no=1";
    $settings_r = mysqli_fetch_assoc(mysqli_query($conn,$settings_query));
    ...
    while($room_data = mysqli_fetch_assoc($room_res))
     {
        // check availaibility filter
    
        if($chk_avail['checkin']!='' && $chk_avail['checkout']!='')
        {
            $tb_query ="SELECT COUNT(*) AS 'total_bookings' FROM booking_order WHERE booking_status=? AND room_id=?
        ...
        $fac_q = mysqli_query($conn,"SELECT f.name, f.id FROM facilities f INNER JOIN rooms_facilities rfac ON f.id = rfac.facilities_id 
        ...
        if(count($facility_list['facilities'])!=$fac_counts){
           continue;
        }
